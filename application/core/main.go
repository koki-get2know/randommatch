...
func emailMatches(c *gin.Context) {
	...
	go func() {
		for _, match := range req.Matches {
			match := match
			err := calendar.SendInvite(&match, adminEmail)
			if err != nil {
				numberOfConversation++
				break // send only once for now
			} else {
				numberOfFailedInvite++
				break
			}
		}
		c.JSON(http.StatusOK, gin.H{
			"message":              "emails have been sent",
			"numConversations":     numberOfConversation,
			"matchingSession":      numberOfMatchingSection,
			"numberOfFailedInvite": numberOfFailedInvite,
		})
	}()
	numberOfMatchingSection++
	c.JSON(http.StatusOK, gin.H{
